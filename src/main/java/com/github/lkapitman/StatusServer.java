...
public final class StatusServer extends JavaPlugin implements Listener {
    ...
    public static StatusServer plugin;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (args.length == 0) {
            if(!sender.hasPermission("ServerStatus.help")) {
                sender.sendMessage("§cServerStatus §8» §6У Вас не достаточно прав.");
                return true;
            }
            sender.sendMessage("§cServerStatus §8» §6Помощь по командами.");
            sender.sendMessage("");
            ...
            switch (args[0].toLowerCase()) {
                case "i":
                case "info":
                    if(!sender.hasPermission("ServerStatus.info")) {
                        sender.sendMessage("§cServerStatus §8» §6У Вас не достаточно прав.");
                        return true;
                    }
                    sender.sendMessage("§cServerStatus §8» §6Информация.");
                    sender.sendMessage("");
                    ...
                    break;
                case "update":
                    if(!sender.hasPermission("ServerStatus.update")) {
                        sender.sendMessage("§cServerStatus §8» §6У Вас не достаточно прав.");
                        return true;
                    }
                    update();
                    sender.sendMessage("§cServerStatus §8» §6Данные успешно обновлены.");
                    break;
                case "rl":
                case "reload":
                    ...
                    this.reloadConfig();
                    loadConfig();
                    sender.sendMessage("§cServerStatus §8» §6Конфигурации успешно перезагружены.");
                    break;
            }
        }
        return false;
